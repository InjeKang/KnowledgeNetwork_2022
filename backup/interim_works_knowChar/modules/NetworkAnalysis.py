...
def knowledge_network(data, knowledgeNetwork):
    if knowledgeNetwork:
        ...
        dc_result["ipc_year"] = dc_result["ipc"] + dc_result["year"]
        sh_result["ipc_year"] = sh_result["ipc"] + sh_result["year"]
...
def network_result(data, network_result_by_year, networkResult):
    if networkResult:  
        # make scolumns: average degree centrality and structural hole of each row
        target_func_dc = partial(network_analysis_by_firm, network_result_by_year)
        result = multi_process_split_designated(data, target_func_dc, split_by_year)
        result.to_excel("data\\01.firm_alliance_v8(knowledge_network).xlsx", index=False)
        return result


def network_analysis_by_firm(network_result_by_year, data):
    # read patent data
    patent_stock = pd.read_pickle("data\\00.patent_stock_v2(four_digitIPC).pkl")
    patent_stock2 = patent_stock[["firm", 10, "four_digitIPC"]] # remove unnecessary column [32]: raw-list of IPCs 
...
def average_degree_centrality(firm_ipc, network_result_by_year):
    selected_rows = network_result_by_year[network_result_by_year["ipc"].isin(firm_ipc)]
    return selected_rows["dc"].mean()


def average_structural_hole(firm_ipc, network_result_by_year):
