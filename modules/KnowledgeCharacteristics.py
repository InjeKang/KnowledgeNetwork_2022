...
def knowledge_characteristics(data, knowledgeCharacteristics):
    if knowledgeCharacteristics:
        # alliance data
        # data = pd.read_pickle("data\\01.firm_alliance_v6(with_patent).pkl")
        # remove irrelevant columns to speed up the analysis
        column_list = ["year", "focal", "focal_ipc"]
        ally_data2 = data[["merged_fpy", "year", "focal", "focal_ipc"]]
        data3 = data.drop(columns = column_list)

        # ally_data2 = ally_data[["merged_fpy", "focal", "year"]]
        # patent data
        patent_data = pd.read_pickle("data\\00.patent_stock_v2(four_digitIPC).pkl")
        ...
        ally_data4.to_excel("data\\01.firm_alliance_v10(knowedge_characteristics).xlsx", index=False)
        return ally_data4
